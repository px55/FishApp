...
namespace WebApplication2
{
    public partial class FishMap : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string latitude = Session["latitude"].ToString();
            string RLong = Session["longitude"].ToString();

            if (!IsPostBack)
            {
                // Current Location
                GLatLng mainLocation = new GLatLng(Convert.ToDouble(latitude), Convert.ToDouble(RLong));
        ...
        {
            Response.Redirect("PlanTrip.aspx");
        }
    }
}